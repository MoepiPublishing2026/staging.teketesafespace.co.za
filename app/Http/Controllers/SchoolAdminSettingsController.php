...
class SchoolAdminSettingsController extends AdminController
{
    public function index()
    {
        ...
        return view('school-admin-settings.index', compact('user'));
    }
    ...
    private function performUpdate(Request $request)
    {
        ...
        if ($request->hasFile('profile_picture')) {
            try {
                ...
                Log::error('School admin profile picture upload failed', [
                    'error' => $e->getMessage(),
                    'user_id' => $user->id,
                ]);
    ...
    private function performDeleteProfilePicture()
    {
        ...
        if ($user->profile_picture) {
            try {
                ...
                Log::error('School admin profile picture delete failed', [
                    'error' => $e->getMessage(),
                    'user_id' => $user->id,
                ]);
