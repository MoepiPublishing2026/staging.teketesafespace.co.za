...
/* ── Filter Panel ──────────────────────────────────────────────── */
.filter-panel { background: #f9fafb; border: 2px solid #e5e7eb; border-radius: 12px; padding: 18px 20px; margin-bottom: 24px; overflow: visible; }
...
.filter-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; align-items: end; overflow: visible; }
...
<div class="main-panel">
    ...
    <main id="main-content">
        ...
        <form id="filterForm" method="GET" action="<?php echo e(url('/national-admin/reports')); ?>">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const subtypesByType = {
        <?php $__currentLoopData = $typeOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php echo e($type->id); ?>: [
                <?php $__currentLoopData = $subtypeOptions->where('abuse_type_id', $type->id); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    { id: <?php echo e($sub->id); ?>, name: "<?php echo e(addslashes($sub->sub_type_name)); ?>" },
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            ],
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    };
    ...
    function filterSubtypes() {
        ...
        const current = "<?php echo e(request('subtype_id')); ?>";
    ...
    filterSubtypes(); // run on page load
...
const allSchools = [
    <?php $__currentLoopData = $schoolOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $school): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        "<?php echo e(addslashes($school->school_name)); ?>",
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
];

const schoolInput   = document.getElementById('schoolInput');

// Create suggestions div at BODY level to escape any overflow clipping
const schoolSuggest = document.createElement('div');
schoolSuggest.id = 'schoolSuggestions';
schoolSuggest.style.cssText = `
    position: fixed;
    background: white;
    ...
    min-width: 200px;
`;
document.body.appendChild(schoolSuggest);

// Position it under the input whenever shown
function positionSuggestions() {
    const rect = schoolInput.getBoundingClientRect();
    schoolSuggest.style.top   = (rect.bottom + window.scrollY + 4) + 'px';
    schoolSuggest.style.left  = rect.left + 'px';
    schoolSuggest.style.width = rect.width + 'px';
}
...
if (!schoolInput) {
    ...
    schoolInput.addEventListener('input', function () {
        ...
        const matches = allSchools.filter(s => s.toLowerCase().includes(query));

        if (matches.length === 0) {
            schoolSuggest.innerHTML = '<div style="padding:8px 12px; font-size:13px; color:#999; font-family:Montserrat,sans-serif;">No schools found</div>';
            positionSuggestions();
        ...
        matches.slice(0, 50).forEach(name => {
            ...
            `;
            item.addEventListener('mouseenter', () => item.style.background = '#f7fcd4');
            ...
            schoolSuggest.appendChild(item);
        });
    ...
    window.addEventListener('scroll', () => {
        if (schoolSuggest.style.display === 'block') positionSuggestions();
    });
    ...
    document.addEventListener('click', function (e) {
        if (!schoolInput.contains(e.target) && !schoolSuggest.contains(e.target)) {
            schoolSuggest.style.display = 'none';
        }
    });
}
...
</html><?php /**PATH /home/teketeq9v8o0/staging.teketesafespace.co.za/resources/views/national-admin-reports/index.blade.php ENDPATH**/ ?>