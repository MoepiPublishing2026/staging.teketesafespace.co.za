<div class="min-h-screen bg-white flex flex-col font-[Montserrat] relative w-full overflow-x-hidden">
   ...
   <div class="flex-grow bg-white pt-28 pb-12 px-4 font-[Montserrat]">
    ...
    <div class="max-w-2xl mx-auto w-full">
        ...
        <div class="bg-white border-4 border-[#c7da30] rounded-2xl shadow-lg overflow-hidden">
            <div class="p-6 border-b-2 border-gray-100">
                ...
                <p class="text-sm text-gray-500">Case Reference: <span class="font-bold text-[#c7da30]">{{ $caseNumber }}</span></p>
            </div>
    ...
    <textarea 
        ...
        wire:key="clarification-input-{{ $showSuccess ? 'success' : 'reset' }}"
        rows="8" 
        placeholder="Provide your detailed statement here..." 
        class="w-full p-4 border-2 border-[#c7da30] rounded-lg shadow-sm outline-none focus:border-[#c7da30] focus:ring-2 focus:ring-[#c7da30] transition-all @error('clarificationText') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror"
    ></textarea>
    @error('clarificationText') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
</div>
