            ...
            <section class="panel heatmap-panel" aria-label="Reports by Province and Type">
                ...
                <div class="heatmap-wrap">
                    <table class="heatmap-table" role="table">
                        ...
                        <tbody>
                            @forelse($heatmapMatrix ?? [] as $province => $row)
                                @php $rowIdx = $loop->index; @endphp
                                <tr>
                                    <th class="heatmap-row">{{ $province }}</th>
                                    @foreach($row as $colIdx => $count)
                                        ...
                                        <td class="heatmap-cell {{ $isDark ? 'heatmap-cell-dark' : '' }} {{ $isHotspot ? 'heatmap-hotspot' : '' }}"
                                            ...
                                            role="button"
                                            tabindex="0"
                                            title="{{ $province }} × {{ $atype }}: {{ $count }} reports ({{ $pct }}% of province) — Click to view reports">
                                            <span class="heatmap-cell-count">{{ $count }}</span>
                                            <span class="heatmap-cell-pct" style="display:none;">{{ $pct }}%</span>
                                        </td>
                                    @endforeach
                                    <td class="heatmap-total-cell">{{ $heatmapRowTotals[$province] ?? 0 }}</td>
                                </tr>
                            @empty
                                <tr>
